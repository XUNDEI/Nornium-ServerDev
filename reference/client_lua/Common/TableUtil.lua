...
local function indexof(array, value, begin, comparator)
    if comparator then
        for i = begin or 1, #array do
            if comparator(array[i], value) then
                return i
            end
        end
    else
...
local function first(array, predictor)
    for i = 1, #array do
        if predictor(array[i]) then
            return i
        end
    end
...
local function any(array, predictor)
    return first(array, predictor) and true
end

local function countBy(array, predictor)
    local count = 0
    for i = 1, #array do
        if predictor(array[i]) then
            count = count + 1
...
local function dump(tb, dump_metatable, max_level)
    ...
    local function _dump(tb, level)
        ...
        for k, v in pairs(tb) do
            ...
            str = str ..
                rep("\t", level + 1) .. "[" .. rep("\"", k_is_str) ..
                (tostring(k) or type(k)) .. rep("\"", k_is_str) .. "]" .. " = "
...
tableex = {}
tableex.list_clear = list_clear
tableex.clear = clear
tableex.read_only = read_only
