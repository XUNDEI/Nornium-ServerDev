...
local function Broadcast(self, e_type, ...)
    ...
    cached_keys = {}
    for k, v in pairs(event) do
        table.insert(cached_keys, k)
    end
    for _, k in ipairs(cached_keys) do
        local v = event[k]
        if v then
            local args = ConcatSafePack(v, SafePack(...))
            local status, resultOrError = xpcall(k, debug.traceback, SafeUnpack(args))
            if not status then
                LOG_ERROR(e_type, resultOrError)
            end
        end
    end
...
local function RemoveListener(self, e_type, e_listener)
    ...
    if event[e_listener] then
        event[e_listener] = nil
    else
        print('---not find this event in MessageManager!')
    end
end
