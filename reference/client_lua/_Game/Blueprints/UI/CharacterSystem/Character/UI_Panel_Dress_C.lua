...
function M:Construct()
    self:InitData()
    self:InitUI()
    MessageManager:GetInstance():AddListener("OnMsg_Res_Character_Change_Skin_Success", self)
end

function M:Destruct()
    MessageManager:GetInstance():RemoveListener("OnMsg_Res_Character_Change_Skin_Success", self)
...
function M:OnMsg_Res_Character_Change_Skin_Success()
    ...
    if self.SelectedRoleId == curCharacterId then
        gameInstance:ChangeDress()
    end
    if self.BackUI and self.BackUI.ChangeIdol and self.SavedDressIdolId > 0 and self.SelectedTabIndex == 1 then
...
function M:InitData()
    ...
    self.UI_DressTab.CityDressTab:SetIsCheckedAndFireEvent(true)
    ...
    if index <= colorListCount and self.ColorSelectedIndex ~= index then
        ...
        if self.BackUI then
            if self.SelectedTabIndex == 0 then
                self.BackUI:ChangeActorMeshEx(colorSkinId)
            elseif self.SelectedTabIndex == 1 then
                self.BackUI:ChangeActorMeshEx(colorSkinId)
                self.BackUI:ChangeIdol(colorSkinId)
            else
                local _, dressId = self:GetCharModeFByCityCharId(colorSkinId)
                if dressId and dressId > 0 then
                    self.BackUI:ChangeActorMeshEx(dressId)
                end
            end
        end
...
function M:RefreshUI(backUI, selectIndex)
    self.BackUI = backUI
    self.SelectedCharacterIndex = selectIndex
...
function M:OnScrollViewSelected(index)
    ...
    if self.SelectedTabIndex == 0 then
        self.CurDressCharId = config.id
    elseif self.SelectedTabIndex == 1 then
        self.CurDressIdolId = config.id
    else
    ...
    if self.BackUI then
        if self.SelectedTabIndex == 0 then
            self.BackUI:ChangeActorMeshEx(config)
        elseif self.SelectedTabIndex == 1 then
            self.BackUI:ChangeActorMesh(config.modelF, nil)
            self.BackUI:ChangeIdol(self.CurDressIdolId)
        else
            local _, dressId = self:GetCharModeFByCityCharId(self.CurDressCityCharId)
            if dressId and dressId > 0 then
                self.BackUI:ChangeActorMeshEx(dressId)
            end
        end
...
function M:RefreshTab(bIsSameTabIndex)
    ...
    if self.SelectedTabIndex == 0 then
        if self.CityCharActor then
            self.CityCharActor:SetActorHiddenInGame(true)
        end
        -- if nil ~= self.CurDressCharId and self.CurDressCharId ~= charId then
        if self.BackUI then
            self.BackUI:ShowActor(true)
            local config = Database.Query("d_char_clothes", charId)
            if config then
                self.BackUI:ChangeActorMeshEx(config)
            end
            self.BackUI:RefreshActorLocation(8)
        end
        -- end
    elseif self.SelectedTabIndex == 1 then
        if self.CityCharActor then
        ...
        -- if nil ~= self.CurDressIdolId and self.CurDressIdolId ~= idolId then
        if self.BackUI then
            local config = Database.Query("d_char_clothes", idolId)
            if config then
                self.BackUI:ShowActor(true)
                self.BackUI:ChangeActorMesh(config.modelF, nil)
                self.BackUI:ChangeIdol(idolId)
            end
            self.BackUI:RefreshActorLocation(9)
        ...
        -- if nil ~= self.CurDressCityCharId and self.CurDressCityCharId ~= cityCharId then
        if self.BackUI then
            local _, dressId = self:GetCharModeFByCityCharId(cityCharId)
            --local config = Database.Query("d_char_clothes", idolId)
            if dressId and dressId > 0 then
                self.BackUI:ChangeActorMeshEx(dressId)
            end
            self.BackUI:RefreshActorLocation(10)
            -- self.BackUI:HideActor(true)
            -- if self.LastRoleId == self.SelectedRoleId then
            --     if self.CityCharActor then
            --         self.CityCharActor:SetActorHiddenInGame(false)
            --     else
            --         self:CreateCityCharActor()
            --     end
            -- else
            --     if self.CityCharActor then
            --         self.CityCharActor:K2_DestroyActor()
    ...
    local dressId = 0
    if self.SelectedTabIndex == 0 then
        dressId = self.SavedDressCharId 
    elseif self.SelectedTabIndex == 1 then
        dressId = self.SavedDressIdolId
    else
        dressId = self.SavedDressCityCharId
    end
    local unlockCount = 0
    ...
    if self.BackUI and self.BackUI.ShowActorWeapon then
        self.BackUI:ShowActorWeapon(self.SelectedTabIndex == 1)
    end
end
...
function M:RefreshDressInfo(config)
    ...
    self.Panel_Color:SetVisibility((self.SelectedTabIndex ~= 1 and colorCount > 0) and UE.ESlateVisibility.SelfHitTestInVisible or UE.ESlateVisibility.Hidden)
    for i = 1, 5 do
        local colorItemUI = self['UI_DressColorItem' .. i]
    ...
    if self.SelectedTabIndex == 0 then
        self.CurDressCharId = selectedRoleIdColorSkinID
    elseif self.SelectedTabIndex == 1 then
        self.CurDressIdolId = selectedRoleIdColorSkinID
    else
        self.CurDressCityCharId = selectedRoleIdColorSkinID
    end
end
...
function M:ReqChangeDressInfo()
    ...
    local msg = {
        character_id      = self.SelectedRoleId,
        character_skin_id = self.SavedDressCharId ~= defaultCharId and self.SavedDressCharId or defaultCharId,
        mecha_skin_id     = self.SavedDressIdolId ~= defaultIdolId and self.SavedDressIdolId or defaultIdolId,
        city_skin_id      = self.SavedDressCityCharId ~= defaultCityCharId and self.SavedDressCityCharId or defaultCityCharId
    }
    CharacterSystem:GetInstance():CacheSkinInfo(msg.character_id, msg.character_skin_id, msg.mecha_skin_id, msg.city_skin_id)
    Client.send('req_character_change_skin', msg)
end
...
function M:GetCharModeFByCityCharId(cityCharId)
    ...
    return charDressConfig.modelF, cityCharId - 2000000
end
...
function M:OnClicked_Btn_Exit()
    if self.BackUI then
        ...
        local charConfig = Database.Query("d_char_clothes", self.SavedDressCityCharId - 2000000)
        if charConfig then
            self.BackUI:ChangeActorMeshEx(charConfig)
        end
    end
...
--主城服装
function M:OnCheckStateChanged_CityDress(isOn)
    if isOn then
...
--战斗服装
function M:OnCheckStateChanged_IdolDress(isOn)
    if isOn then
...
--机甲涂装
function M:OnCheckStateChanged_MeshDressTab(isOn)
    if isOn then
...
function M:IsUnlockSkinId(roleId, type, skinId)
    ...
    -- end
    --判断角色皮肤解锁
    local _, isUnlock = UIUtils.CharSkinIsUnlock(roleId, type, skinId)
    return isUnlock
end
