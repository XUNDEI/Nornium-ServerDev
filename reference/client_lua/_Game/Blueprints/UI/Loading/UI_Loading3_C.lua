...
--构造函数
function UI_Loading3_C:Construct()
...
-- function UI_Loading3_C:RefreshUI(curCount, AllCount)
...
function UI_Loading3_C:RefreshUI(delayTimeToDestroy)
...
function UI_Loading3_C:DelayDestroy()
    -- 取消后不再接受新的Delay
    if self.cancel_delay_destroy then return end
    if self.DelayDestroyHandle then
        UE.UKismetSystemLibrary.K2_ClearAndInvalidateTimerHandle(self, self.DelayDestroyHandle)
        self.DelayDestroyHandle = nil
    end
...
function UI_Loading3_C:CancelDelayDestroy()
...
function UI_Loading3_C:OnDelayDestroy()
    if not UE.UKismetSystemLibrary.IsValid(self) then return end
    local gameInstance = UE.UGameplayStatics.GetGameInstance(self)
    gameInstance:RemoveUMG("UI_Loading3")
end
