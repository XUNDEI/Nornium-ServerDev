...
function M:InitActivityInfo(isInit)
    ...
    local passed_level_id = copyInfo.passed_level_id
    ...
    for _, config in pairs(d_levels_challenge) do
        if config.levelTypes == UIUtils.ECopyType.ECopyType_Challenge then
...
function M:OnClicked_Btn_Fight()
    ...
    -- if saveGameSpeak.StoryFinished then
        gameInstance.UIName = 'UI_Challenge_Copy'
