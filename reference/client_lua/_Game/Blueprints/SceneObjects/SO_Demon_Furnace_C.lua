    ...
    if playerController then
        ...
        if player then
            if player == OtherActor then
                ...
                UIManager:GetInstance():AddInteractOption(self, 245, function()
...
function M:OnClickOptionItem()
    UIManager:GetInstance():RemoveInteractOptionByObject(self)
    -- self.Overridden.OnClickOptionItem(self)
    local gameInstance = UE.UGameplayStatics.GetGameInstance(self)
    local isOpen, config = gameInstance:OpenLinkEx(9005)
